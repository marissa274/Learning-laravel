...
<x-layout>
    ...
    <p>Cette page permet de modifier une publication </p>
    <form method="POST" action="/posts/{{ $post->id }}">
        @method("PUT")
        @csrf
        <div>
            <label for="title">Titre</label>
            <input type="text" id="title" name="title" placeholder="Ma publication"  value="{{ old('title') ?? $post->title}}" class="@error('title') is-invalid  @enderror">
            @error('title')
